  ...
  head: () => ({
    meta: [
      { title: "Sign in — FedRetina" },
      {
        name: "description",
        content:
          "Sign in or create your FedRetina account to grade retina scans.",
      },
...
const inputClass =
  "mt-1 w-full rounded-md border border-border bg-surface px-3 py-2.5 text-sm text-foreground outline-none focus:border-primary focus:ring-2 focus:ring-ring";
function AuthPage() {
      ...
      <div className="fr-rise w-full max-w-md rounded-lg border border-border bg-surface p-7 shadow-md">
        ...
        <form className="mt-6 space-y-4" onSubmit={handleSubmit}>
          ...
          {notice && (
            <p
              role="status"
              className="rounded-md bg-primary-light px-3 py-2 text-sm text-primary"
            >
              {notice}
            </p>
          )}
          ...
          <button
            type="submit"
            disabled={busy}
            className="min-h-11 w-full rounded-md bg-primary text-sm font-semibold text-primary-foreground transition-colors hover:bg-primary/90 disabled:opacity-60"
          >
            {busy
              ? "Please wait…"
              : mode === "signin"
