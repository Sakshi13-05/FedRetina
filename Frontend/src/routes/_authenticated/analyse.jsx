...
import { Upload, ImageIcon, RefreshCw, CheckCircle2, AlertTriangle } from "lucide-react";
...
import { GRADE_LABELS, GRADE_COLOR_CLASS, gradeProbabilities } from "@/lib/fedretina/mockData";

export const Route = createFileRoute("/_authenticated/analyse")({
  head: () => ({
    meta: [
      { title: "Analyse a scan — FedRetina" },
      { name: "description", content: "Upload one retina photo and get a severity reading with a confidence score." },
      { property: "og:title", content: "Analyse a scan — FedRetina" },
      { property: "og:description", content: "Upload one retina photo and get a severity reading with a confidence score." },
    ],
  }),
...
function AnalysePage() {
  ...
  function pickFile(nextFile) {
    ...
    if (nextFile.size > 12 * 1024 * 1024) {
      setError("That photo is larger than 12 MB. Please choose a smaller file.");
      return;
    }
    setFile(nextFile);
    ...
    <AppShell>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[1.1fr_1fr]">
        ...
        <Reveal className="rounded-lg border border-border bg-surface p-5 shadow-sm">
          <h2 className="text-sm font-semibold text-foreground">Retina photo</h2>
          ...
          >
            {previewUrl ? (
              <span className="relative block w-full overflow-hidden rounded-md">
                <img src={previewUrl} alt="Selected retina photo" className="mx-auto max-h-64 rounded-md object-contain" />
                {running && (
                  <span aria-hidden className="fr-scanline absolute inset-x-0 top-0 h-10 bg-primary/25" />
                )}
              </span>
            ) : (
              <>
                <ImageIcon size={28} aria-hidden className="text-primary" />
                <span className="text-sm font-medium text-foreground">Drop a photo here, or choose a file</span>
                <span className="text-xs text-muted-foreground">JPG or PNG, up to 12 MB</span>
              </>
            )}
            <input
              ref={inputRef}
          ...
          {file && <p className="mt-3 truncate text-xs text-muted-foreground">{file.name}</p>}
          {error && (
        ...
        <Reveal delay={120} className="space-y-6">
              ...
              {STAGES.map((label, index) => {
                ...
                return (
                  <li key={label} className="flex items-center gap-3 text-sm">
                    ...
                    {active && <span className="ml-auto"><LivePulse label="" tone="primary" /></span>}
                  </li>
                );
              })}
            </ol>
          ...
          {result ? (
            <div className="fr-rise rounded-lg border border-border bg-surface p-5 shadow-md">
              ...
              <p className={`mt-3 text-3xl font-bold tracking-tight ${GRADE_COLOR_CLASS[result.grade]}`}>
                Grade {result.grade} · {GRADE_LABELS[result.grade]}
              </p>
              <p className="mt-1 text-sm text-secondary-text">{NEXT_STEPS[result.grade]}</p>

              <div className="mt-5 space-y-2">
                {gradeProbabilities(result.grade, result.confidence).map((row) => (
            ...
            <RotatingQuote
              items={[
                { text: "Good lighting and a steady camera give the clearest reading.", source: "Imaging tip" },
                { text: "Photos stay on this site — only anonymous learning is shared.", source: "Privacy note" },
                { text: "Low certainty means a colleague should look, not that it is wrong.", source: "Clinical safety" },
              ]}
            />
          )}
        </Reveal>
