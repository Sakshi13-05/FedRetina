...
import { MOCK_PREDICTIONS, GRADE_LABELS, GRADE_COLOR_CLASS } from "@/lib/fedretina/mockData";

export const Route = createFileRoute("/_authenticated/history")({
  head: () => ({
    meta: [
      { title: "Scan history — FedRetina" },
      { name: "description", content: "Search and filter every retina scan graded at your site." },
      { property: "og:title", content: "Scan history — FedRetina" },
      { property: "og:description", content: "Search and filter every retina scan graded at your site." },
    ],
  }),
...
function HistoryPage() {
  ...
  const rows = useMemo(() => {
    return MOCK_PREDICTIONS.filter((p) => {
      const matchesQuery = p.patientId.toLowerCase().includes(query.trim().toLowerCase());
    ...
    <AppShell>
      ...
      <Reveal className="mt-6 flex flex-wrap items-center gap-3 rounded-lg border border-border bg-surface p-4 shadow-sm">
        <label className="relative flex min-w-56 flex-1 items-center">
          <Search size={16} aria-hidden className="pointer-events-none absolute left-3 text-muted-foreground" />
      ...
      {/* Table */}
      <Reveal delay={100} className="mt-6 overflow-hidden rounded-lg border border-border bg-surface shadow-sm">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[640px] text-left text-sm">
            <caption className="sr-only">Past retina scans</caption>
            <thead>
              <tr className="border-b border-border">
                <th scope="col" className="label-xs px-4 py-3">Patient</th>
                <th scope="col" className="label-xs px-4 py-3">Severity</th>
                <th scope="col" className="label-xs px-4 py-3">Certainty</th>
                <th scope="col" className="label-xs px-4 py-3">Status</th>
                <th scope="col" className="label-xs px-4 py-3">When</th>
                <th scope="col" className="label-xs px-4 py-3 text-right">Open</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((p, index) => (
                ...
                >
                  <td className="px-4 py-3 font-medium text-foreground">{p.patientId}</td>
                  <td className={`px-4 py-3 font-semibold ${GRADE_COLOR_CLASS[p.grade]}`}>
                    Grade {p.grade} · {GRADE_LABELS[p.grade]}
                  </td>
                  <td className="px-4 py-3 text-secondary-text">{Math.round(p.confidence * 100)}%</td>
                  <td className="px-4 py-3">
                    ...
                    >
                      {p.status === "needs_review" ? "Needs review" : "Reviewed"}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-muted-foreground">{formatWhen(p.createdAt)}</td>
